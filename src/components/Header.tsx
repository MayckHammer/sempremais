...
export function Header() {
  ...
  const MobileMenu = () => <div className="flex flex-col gap-4 p-4">
      {user ? <>
          {user.role === 'client' && <Link to="/cliente" onClick={() => setOpen(false)}>
              <Button variant="ghost" className="w-full justify-start">
                <User className="w-5 h-5 mr-3 text-primary" />
                Minha Área
              </Button>
            </Link>}
          {user.role === 'provider' && <Link to="/prestador" onClick={() => setOpen(false)}>
              <Button variant="ghost" className="w-full justify-start">
  ...
  return <header className="bg-card/80 backdrop-blur-xl border-b border-border sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-3">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 sm:gap-4">
            <img src={logo} alt="Mi Rebok" className="w-24 h-24 sm:w-32 sm:h-32 rounded-xl sm:rounded-2xl object-contain -my-6 sm:-my-9" />
            <div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden md:flex items-center gap-2">
              {user ? <>
                  {user.role === 'client' && <Link to="/cliente">
                      <Button variant="ghost" size="icon" className="rounded-xl">
                        <User className="w-5 h-5 text-primary" />
                      </Button>
                    </Link>}
                  {user.role === 'provider' && <Link to="/prestador">
                      <Button variant="ghost" size="icon" className="rounded-xl">
